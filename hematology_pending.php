...
<html lang = "en">
	...
	<div class = "well">
		...
		<div class = "panel panel-success">
			<div class = "panel-heading">
				<label>HEMATOLOGY REQUEST</label>
				<a style = "float:right; margin-top:-4px;" href = "view_hematology.php?itr_no=<?php echo $_GET['itr_no']?>" class = "btn btn-info"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
			</div>
			...
			<!-- <?php
				...
				$f1 = $q1->fetch_array();
				$q = $conn->query("SELECT * FROM `complaints` WHERE `section` = 'Hematology' && `itr_no` = '$id' ORDER BY `status` DESC") or die(mysqli_error());
				while($f = $q->fetch_array()){
					if($f['status'] == 'Pending'){
						echo "<label style = 'color:#3399f3;'>".$f1['firstname']." ".$f1['lastname']."</label>"."<textarea  style = 'resize:none;' disabled = 'disabled' class = 'form-control'>".$f['remark']."</textarea>".$f['date']."<br /><a class = 'btn btn-primary' href = 'hematology_not.php?itr_no=".$id."&comp_id=".$f['com_id']."'><span class = 'glyphicon glyphicon-check'></span> Confirm</a><br /><br />";
					}else{
    ...
    while($f = $q->fetch_array()){
        if($f['status'] == 'Pending'){
            ...
            echo $f['date'] . "<br>";
            echo "<a class='btn btn-primary' href='hematology_not.php?itr_no=" . $id . "&comp_id=" . $f['com_id'] . "'><span class='glyphicon glyphicon-check'></span> Confirm</a><br /><br />";
        } else {
            echo "<label style='color:#3399f3;'>Patient Name: </label>" . $f1['firstname'] . " " . $f1['lastname'] . "<br>";
            echo "<label style='color:#3399f3;'>Complaints: </label>" . "<textarea style='resize:none;' disabled='disabled' class='form-control'>" . $f['complaints'] . "</textarea><br>";
			echo "<label style='color:#3399f3;'>Objective: </label>" . "<textarea  style='resize:none;' disabled='disabled' class='form-control'>" . $f['remark'] . "</textarea><br>";
